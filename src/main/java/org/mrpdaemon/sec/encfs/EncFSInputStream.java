...
public class EncFSInputStream extends FilterInputStream {
   ...
   * @param volumePath Volume path of the file being decrypted (needed for
   *                   externalIVChaining)
   */
  public EncFSInputStream(EncFSVolume volume, InputStream in,
    ...
    if (config.isUseUniqueIV()) {
      ...
      try {
        this.fileIv = StreamCryptography.streamDecode(volume, initIv, fileHeader);
  ...
  private int readBlock() throws IOException, EncFSCorruptDataException,
    ...
    if (bytesRead == blockSize) { // block decode
      ...
      try {
        blockBuf = StreamCryptography.streamDecode(volume, getBlockIV(), cipherBuf, 0, bytesRead);
      } catch (InvalidAlgorithmParameterException e) {
        e.printStackTrace();
      } catch (IllegalBlockSizeException e) {
        throw new EncFSCorruptDataException(e);
