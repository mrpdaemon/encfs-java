...
public class EncFSFile {
  ...
  public EncFSInputStream openInputStream() throws EncFSCorruptDataException, EncFSUnsupportedException, IOException {
    return new EncFSInputStream(volume, volume.getFileProvider().openInputStream(getEncryptedPath()), getPath());
  }
  ...
  public boolean copy(EncFSFile dstPath) throws IOException {
    if (isDirectory()) {
      ...
      if (volume.getVolumeConfiguration().isSupportedExternalIVChaining()) {
        ...
        return volume.getFileProvider().copy(getEncryptedPath(), dstPath.getEncryptedPath());
      }
    }
  }
}