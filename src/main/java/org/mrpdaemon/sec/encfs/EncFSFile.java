...
public class EncFSFile {
   ...
   *
   * @param volume         The volume that contains the file
   * @param plainFileInfo  EncFSFileInfo representing the cleartext file (decoded)
   * @param cipherFileInfo EncFSFileInfo representing the ciphertext file (encoded)
   */
   ...
   * @throws IOException               FileProvider returned I/O error
   */
  ...
  public boolean copy(EncFSFile dstPath) throws IOException {
    if (this.isDirectory()) {
      ...
      if (volume.getConfig().isExternalIVChaining()) {
				 ...
				 */
        try {
          EncFSUtil.copyWholeStreamFinallyClose(openInputStream(), dstPath.openOutputStream(getLength()));
