      ...
      for (int i = 0; i < encFsFiles.length; i++) {
        ...
        if (decodedFsFile.isDirectory()) {
          ...
          File t = File.createTempFile(this.getClass().getName(),
              ".tmp");
          try {
            EncFSUtil.copyWholeStreamFinallyClose(new EncFSFileInputStream(
                encFsFile),
                new EncFSOutputStream(encFsDir.getVolume(), new BufferedOutputStream(new FileOutputStream(t)), encFsFile.getPath()));
